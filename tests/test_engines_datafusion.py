...
class TestDataFusionLoader:
    ...
    def test_load_csv(self, tmp_path):
        ...
        frame = DataFusionLoader().load(req)
...
class TestDataFusionWriter:
    def test_can_write_false_without_destination(self):
        req = _req("x.csv", FileFormat.CSV)
        assert not DataFusionWriter().can_write(req)
    ...
    def test_round_trip(self, tmp_path, fmt):
        ...
        frame = DataFusionLoader().load(req)
        DataFusionWriter().write(frame, req)

        assert dst.exists()
        result = DataFusionLoader().load(_req(str(dst), fmt))
    ...
    def test_append_adds_rows(self, tmp_path):
        ...
        DataFusionWriter().write(DataFusionLoader().load(first_req), first_req)
        ...
        DataFusionWriter().write(DataFusionLoader().load(append_req), append_req)

        result = DataFusionLoader().load(_req(str(dst), FileFormat.CSV))
