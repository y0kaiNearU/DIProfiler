...
class TestDaskWriter:
    def test_can_write_false_without_destination(self):
        req = _req("x.csv", FileFormat.CSV)
        assert not DaskWriter().can_write(req)
    ...
    def test_round_trip(self, tmp_path, dask_client, fmt):
        src = tmp_path / "in.csv"
        _write_file(src, FileFormat.CSV, [(1, "x"), (2, "y")])
        dst = tmp_path / f"out_{fmt.value}"
        req = _req(str(src), FileFormat.CSV, str(dst), fmt)

        loader = DaskLoader(factory=lambda: dask_client)
        writer = DaskWriter(factory=lambda: dask_client)
    ...
    def test_append_adds_rows(self, tmp_path, dask_client):
        src = tmp_path / "in.csv"
        _write_file(src, FileFormat.CSV, [(1, "x"), (2, "y")])
