...
class TestPandasWriter:
    def test_can_write_false_without_destination(self):
        req = _req("x.csv", FileFormat.CSV)
        assert not PandasWriter().can_write(req)
    ...
    def test_round_trip(self, tmp_path, fmt):
        ...
        frame = PandasLoader().load(req)
        PandasWriter().write(frame, req)

        assert dst.exists()
        result = PandasLoader().load(_req(str(dst), fmt))
    ...
    def test_append_adds_rows(self, tmp_path):
        ...
        PandasWriter().write(PandasLoader().load(first_req), first_req)
        ...
        PandasWriter().write(PandasLoader().load(append_req), append_req)

        result = PandasLoader().load(_req(str(dst), FileFormat.CSV))
