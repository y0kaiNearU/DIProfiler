...
class TestArrowWriter:
    def test_can_write_false_without_destination(self):
        req = _req("x.csv", FileFormat.CSV)
        assert not ArrowWriter().can_write(req)
    ...
    def test_round_trip(self, tmp_path, fmt):
        ...
        frame = ArrowLoader().load(req)
        ArrowWriter().write(frame, req)

        assert dst.exists()
        result = ArrowLoader().load(_req(str(dst), fmt))
    ...
    def test_append_adds_rows(self, tmp_path):
        ...
        ArrowWriter().write(ArrowLoader().load(first_req), first_req)
        ...
        ArrowWriter().write(ArrowLoader().load(append_req), append_req)

        result = ArrowLoader().load(_req(str(dst), FileFormat.CSV))
