...
class TestPolarsWriter:
    def test_can_write_false_without_destination(self):
        req = _req("x.csv", FileFormat.CSV)
        assert not PolarsWriter().can_write(req)
    ...
    def test_round_trip(self, tmp_path, fmt):
        ...
        frame = PolarsLoader().load(req)
        PolarsWriter().write(frame, req)

        assert dst.exists()
        result = PolarsLoader().load(_req(str(dst), fmt))
    ...
    def test_append_adds_rows(self, tmp_path):
        ...
        PolarsWriter().write(PolarsLoader().load(first_req), first_req)
        ...
        PolarsWriter().write(PolarsLoader().load(append_req), append_req)

        result = PolarsLoader().load(_req(str(dst), FileFormat.CSV))
        rows = sorted(result.collect().rows(named=True), key=lambda r: r["a"])
