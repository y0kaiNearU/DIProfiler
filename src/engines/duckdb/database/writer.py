...
def write(conn: Any, frame: nw.LazyFrame, dest: DatabaseSource) -> None:
    ...
    query = dest.queries.get(EngineType.DUCKDB)
    select = query if query else "SELECT * FROM _frame"
    try:
        if dest.write_mode == WriteMode.OVERWRITE:
            conn.execute(f"DELETE FROM {target}")
        conn.execute(f"INSERT INTO {target} {select}")
    except Exception as e:
        location = f"'{dest.schema}.{dest.table_name}'" if not query else "custom query"
        raise RuntimeError(
